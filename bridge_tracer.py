...
ПОЧЕМУ ПОСТ-BRIDGE ОБХОД РЕАЛИЗОВАН ЗАНОВО, А НЕ ЧЕРЕЗ Flow & Hop Tracer
----------------------------------------------------------------------------
В исходной AML-платформе, из которой сюда переиспользованы evm_adapter/
tron_adapter (см. aml/README.md), был Flow & Hop Tracer — BFS по исходящим
переводам с poison/taint-разметкой, но рассчитанный на другой сценарий:
пишет рёбра в PostgreSQL (evidence_log/flow_edges/seed_registry), резолвит
свопы через TheGraph (нужен THEGRAPH_API_KEY), стоп-условие "known_exchange"
читает из label_cache, которую заполняет отдельный Attribution Engine. Для
ЭТОГО тестового задания — самодостаточный Layer 1 без внешних БД/сервисов,
стоп-условия по маленькому статическому реестру (known_contracts.py) —
переиспользовать этот компонент было бы неверно: он тянет за собой Postgres
+ TheGraph там, где задача явно просит "MVP, без production-грейда
полноты". Пост-bridge обход здесь реализован заново, проще и без внешних
зависимостей, кроме тех же трёх адаптеров — поэтому исходный Flow & Hop
Tracer (вместе с остальной частью платформы, не нужной этому заданию) в
этот репозиторий не включён вовсе.
...
async def trace_full_path(
    ...
    Args:
        ...
        mode: пробрасывается в метаданные результата для совместимости с
            остальной AML-платформой (её tool-calling обёртки тоже принимают
            mode как параметр — см. aml/README.md про то, что из платформы
            сюда переиспользовано, а что нет); в этой версии MVP поведение
            самого трейсера от mode не зависит — все режимы используют
            одинаковую глубину/логику обхода.
